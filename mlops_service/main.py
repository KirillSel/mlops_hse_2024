...
async def train_model(request: TrainRequest):
    """
    Обучает модель на основе переданного типа, гиперпараметров и количества признаков.
    
    Args:
        request (TrainRequest): Данные запроса для обучения модели, включая тип, гиперпараметры и количество признаков.
    
    Returns:
        dict: Статус обучения и идентификатор обученной модели.
    """
    logger.info(f"Training model of type {request.model_type} with hyperparameters {request.hyperparameters} and {request.num_features} features")
    ...
    return {"status": "Model trained", "model_id": model_id, "num_features": request.num_features}
